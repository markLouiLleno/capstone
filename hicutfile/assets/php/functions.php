...
function validateSignupForm($form_data) {
    ...
    if (!$first_name) {
        ...
        return $response; // Exit early if first name is not given
    } elseif (!preg_match('/^[A-Za-z]+$/', $first_name)) {
        $response['msg'] = "First name should contain only letters";
        $response['status'] = false;
        $response['field'] = 'first_name';
        return $response; // Exit early if first name contains invalid characters
    }
    ...
    if (!$last_name) {
        ...
        return $response; // Exit early if last name is not given
    } elseif (!preg_match('/^[A-Za-z]+$/', $last_name)) {
        $response['msg'] = "Last name should contain only letters";
        $response['status'] = false;
        $response['field'] = 'last_name';
        return $response; // Exit early if last name contains invalid characters
    }
...
function createUser($data)
{
    ...
    $profession = mysqli_real_escape_string($db, $data['profession']);
    $started_date = mysqli_real_escape_string($db, $data['started_date']);
    $password = mysqli_real_escape_string($db, $data['password']);
    $password = md5($password);

    // Using prepared statement to prevent SQL injection
    $query = "INSERT INTO users (profession, location, first_name, last_name, gender, email, username, password, user_role, started_date, ac_status) ";
    $query .= "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)";
    ...
    // Bind parameters
    $stmt->bind_param("ssssisssss", $profession, $location, $first_name, $last_name, $gender, $email, $username, $password, $data['user_role'], $started_date);
...
function validateUpdateForm($form_data,$image_data){
    ...
    $response['status']=true;
       ...
       if($image_data['name']){
        ...
        if($size>50000){
            $response['msg']="upload image less then 50 mb";
            $response['status']=false;
            $response['field']='profile_pic';
        }
       }
